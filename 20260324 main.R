...
# source( "S:/Users/AnthonyD/Medicare Tracker/programs/county-level plan enrollment/20260324 main.R" , echo = TRUE )
# this script creates four tables for 2026: ma & pdp for fall availability then spring enrollment

# to make availability & enrollment files for 2025, use
# source( "S:/Users/AnthonyD/Medicare Tracker/programs/county-level plan enrollment/20250306 main.R" , echo = TRUE )

# to make enrollment files before 2025, use this script with updated recodes (such as firm identification removal)
# S:\Users\AnthonyD\Medicare Tracker\programs\county-level plan enrollment\20240624 main.R

# to make availability files before 202409, use this script with updated recodes (such as firm identification removal)
# S:\Users\AnthonyD\Medicare Tracker\programs\county-level plan availability\20241022 main.R
...
if( Sys.Date() > '2026-07-01' ) stop( "alaska not available in the march 2026 penetration files (see readme)" )

# https://www.cms.gov/data-research/statistics-trends-and-reports/medicare-advantagepart-d-contract-and-enrollment-data/ma-state/county-penetration/ma-state-county-penetration-2026-03
ma_penetration_fn <- "R:/Medicare Tracker/input/state_county_penetration_ma_2026_03/State_County_Penetration_MA_2026_03.csv"

# https://www.cms.gov/data-research/statistics-trends-and-reports/medicare-advantagepart-d-contract-and-enrollment-data/pdp-state/county-penetration/pdp-state-county-penetration-2026-03
pdp_penetration_fn <- "R:/Medicare Tracker/input/state_county_penetration_pdp_2026_03/State_County_Penetration_PDP_2026_03.csv"
...
medicare_enrollment_dashboard_fn <- "R:/Medicare Tracker/input/Medicare Monthly Enrollment Data_December 2025.csv"

dashboard_save_fn <- "R:/Medicare Tracker/output/202512 dashboard beneficiaries by fips.rds"

# https://www.cms.gov/medicare/coverage/prescription-drug-coverage
combined_landscape_fn <- "R:/Medicare Tracker/input/CY2026_Landscape_202603/CY2026_Landscape_202603.csv"


# https://www.cms.gov/data-research/statistics-trends-and-reports/medicare-advantagepart-d-contract-and-enrollment-data/monthly-enrollment-contract/plan/state/county/monthly-enrollment-cpsc-2026-03
cpsc_enrollment_fn <- "R:/Medicare Tracker/input/CPSC_Enrollment_2026_03/CPSC_Enrollment_Info_2026_03.csv"
cpsc_contract_fn <- "R:/Medicare Tracker/input/CPSC_Enrollment_2026_03/CPSC_Contract_Info_2026_03.csv"
...
ma_contract_directory_fn <- "R:/Medicare Tracker/input/MA_Plan_Directory_2026_03/MA_Contract_directory_2026_03.csv"
...
pdp_contract_directory_fn <- "R:/Medicare Tracker/input/PDP_Plan_Directory_2026_03/PDP_Contract_directory_2026_03.csv"
...
# # # # # # # # # # # # # # # #
# ADD IN ALASKA & CONNECTICUT #
# # # # # # # # # # # # # # # #


if( Sys.Date() > '2026-07-01' ){
	
	stop( "review alaska & connecticut mismatches" )

	# text from the march 2026 penetration file readmes:

	# the december 2025 medicare dashboard has the new counties, the march 2026 cpsc has the old counties, the march 2026 penetration file has neither:

		# The state of Connecticut [CT] has transitioned existing counties for Connecticut to county-equivalent geographic units.  
		# This change created the need to update associated FIPS and SSA codes for the new areas. 
		# We are working to move all Connecticut county data to the designated county-equivalent geographic units.  
		# As of this month, the FIPS and SSA codes for the new Connecticut geographic units have not been updated in the CMS data. 
		# For this reason, data for CT are excluded from the Penetration files for this month because the penetration statistics 
		# are not accurate for CT until this information is updated. 


	# the december 2025 medicare dashboard has the new counties, the march 2026 cpsc has the old counties, the march 2026 penetration file has neither:

		# The State of Alaska [AK] has redistricted five of its counties: Hoonah-Angoon, Petersburg, Prince of Wales-Hyder, 
		# Skagway, Wrangell City and Borough. In addition, the County of Valdez was split into Chugach and Copper River.  
		# The data for Alaska do not reflect the changes in the new county designations because the FIPS and SSA codes 
		# for the new Alaska counties have not been updated in the CMS data. For this reason, data for AK are excluded for this month.


}	

# load old alaska & connecticut ssa & fips codes from the 2024 nber xwalk
akct_xw_df <- subset( nber_xw_df , state_name %in% c( 'ALASKA' , 'CONNECTICUT' ) )
akct_xw_df[ akct_xw_df[ , 'state_name' ] == 'CONNECTICUT' , 'state_name' ] <- 'Connecticut'
akct_xw_df[ akct_xw_df[ , 'state_name' ] == 'ALASKA' , 'state_name' ] <- 'Alaska'

akct_xw_df[ , 'county' ] <- 
	gsub(
		"(?<=\\b)([a-z])" , 
		"\\U\\1" , 
		tolower( akct_xw_df[ , 'countyname_fips' ] ) , 
		perl = TRUE
	)
	
akct_xw_df <- akct_xw_df[ c( 'state_name' , 'county' , 'fipscounty' , 'ssa_code' ) ]
names( akct_xw_df ) <- c( 'state_name' , 'county' , 'fips' , 'ssa' )
akct_xw_df[ , 'ssa' ] <- as.numeric( akct_xw_df[ , 'ssa' ] )
akct_xw_df[ , c( 'eligibles' , 'ma_enrolled' , 'pdp_enrolled' ) ] <- NA
# one record per alaska & connecticut county, with missing penetration file variables

if( Sys.Date() > '2026-07-01' ) stop( "hardcoding/harmonizing alaska" ) else {

	# remove counties without names
	akct_xw_df <- subset( akct_xw_df , county != '' )

	# keep fips = 2013 + ssa = 2013 and also fips = 2016 + ssa = 2016 but not their intersecting records
	akct_xw_df <- akct_xw_df[ !( akct_xw_df[ , 'fips' ] %in% 2013 & akct_xw_df[ , 'ssa' ] %in% 2016 ) , ]
	akct_xw_df <- akct_xw_df[ !( akct_xw_df[ , 'fips' ] %in% 2016 & akct_xw_df[ , 'ssa' ] %in% 2013 ) , ]

	akct_xw_df[ akct_xw_df[ , 'county' ] == 'Lake And Peninsula' , 'county' ] <- 'Lake and Peninsula'
	akct_xw_df[ akct_xw_df[ , 'county' ] == 'Bristol Bay Borough' , 'county' ] <- 'Bristol Bay'
	akct_xw_df[ akct_xw_df[ , 'county' ] == 'Kodiak Island Borough' , 'county' ] <- 'Kodiak Island'
	akct_xw_df[ akct_xw_df[ , 'county' ] == 'North Slope Borouh' , 'county' ] <- 'North Slope'
	akct_xw_df[ akct_xw_df[ , 'county' ] == 'Northwest Artic Borough' , 'county' ] <- 'Northwest Arctic'
	akct_xw_df[ akct_xw_df[ , 'county' ] == 'Yakutat Borough' , 'county' ] <- 'Yakutat'
...
if( Sys.Date() > '2026-07-01' ){
	...
	stop( "review the two penetration files and decide whether this append is still necessary or if the recode below works instead" )
...
if( Sys.Date() > '2026-07-01' ) stop( "these penetration files have no connecticut records, try subset(map_df,State.Name=='Connecticut') and subset(pdpp_df,State.Name=='Connecticut')" )

if( Sys.Date() > '2026-07-01' ) stop( "these penetration files have no alaska records, try subset(map_df,State.Name=='Alaska') and subset(pdpp_df,State.Name=='Alaska')" )
...
if( Sys.Date() > '2026-07-01' ){
	...
	stop( "review the two penetration files and decide whether to re-implement this recode or the append above" )
...
if( Sys.Date() > '2026-07-01' ){
	...
	stop( "review the two penetration files and decide whether to re-implement this check" )
...
# with the september 2025 penetration files, only three MP counties collapse into one
# stopifnot( nrow( pen_df ) + 2 == before_nrow )
# with the march 2026 penetration files, MP not included and no other reductions
stopifnot( nrow( pen_df ) == before_nrow )


if( Sys.Date() > '2026-07-01' ) stop( "appending blank alaska, connecticut, and northern mariana islands records" )
pen_df <- rbind( pen_df , akct_xw_df )
...
if( Sys.Date() > '2026-07-01' ) stop( "March 2026 denominator?" )
medicare_enrollment_dashboard_df <- subset( medicare_enrollment_dashboard_df , YEAR == 2025 & MONTH == 'December' )
...
if( Sys.Date() > '2026-07-01' ) stop( "removing connecticut but not alaska total" )
...
if( Sys.Date() > '2026-07-01' ) stop( "appending blank connecticut but not alaska records" )

dashboard_connecticut_df <- 

	transform( 
	
		akct_xw_df[ akct_xw_df[ , 'fips' ] %in% 9000:9999 , c( 'state_name' , 'county' , 'fips' ) ] , 
...
if( Sys.Date() > '2026-07-01' ) stop( "less than ten alaska counties with small populations still do not match" )
...
if( Sys.Date() > '2026-07-01' ){
	...
	stop( "some contracts in the landscape file do not have a match in the contract directory.  this should be allowed for fall availability but not spring enrollment - one NY dual snp also missing" )
...
# every record except for connecticut, alaska, and the northern mariana islands has an eligibles count
stopifnot( all( !is.na( lsc_state_fips_df[ !( lsc_state_fips_df[ , 'state_name' ] %in% c( 'Alaska' , 'Connecticut' , 'Northern Mariana Islands' ) ) , 'eligibles' ] ) ) )
...
if( Sys.Date() > '2026-07-01' ) stop( "note missing eligibles, ma_enrolled, and pdp_enrolleed in connecticut, alaska, and the northern mariana islands" )
...
if( Sys.Date() > '2026-07-01' ) stop( "review filepaths to be saved" )

# one record per plan-county of ma availability in year

# this prior save used the landscape file for the 2026 open enrollment period released by cms in september 2025
...
# this prior save used the landscape file for the 2026 open enrollment period released by cms in march 2026
saveRDS( ma_availability_df , file = paste0( "R:/Medicare Tracker/output/202603 medicare advantage county plan availability.rds" ) )
...
# # # # # # # # # # # # #
# second of four saves  #
# # # # # # # # # # # # #

if( Sys.Date() > '2026-07-01' ) stop( "review filepaths to be saved" )

# one record per plan-county of pdp availability in year

# this prior save used the landscape file for the 2026 open enrollment period released by cms in september 2025
# saveRDS( pdp_availability_df , file = paste0( "R:/Medicare Tracker/output/" , year - 1 , "09 pdp county plan availability.rds" ) )

# this prior save used the landscape file for the 2026 open enrollment period released by cms in march 2026
saveRDS( pdp_availability_df , file = paste0( "R:/Medicare Tracker/output/202603 pdp county plan availability.rds" ) )
...
# # # # # # # # # # # # # # # # # # # #
# end of availability table creation  #
# start of enrollment table creation  #
# # # # # # # # # # # # # # # # # # # #
...
# # # # # # # # # # # # # # # # # # # #
# merge enrollment and contract info  #
# # # # # # # # # # # # # # # # # # # #


# cpsc enrollment
cpsc_enrollment_df <- read.csv( cpsc_enrollment_fn )

names( cpsc_enrollment_df ) <- c( 'contract_id' , 'plan_id' , 'ssa' , 'fips' , 'stateab' , 'county' , 'enrolled' )

# plan-county records with less than 11 are squelched in the file
cpsc_enrollment_df[ cpsc_enrollment_df[ , 'enrolled' ] == '*' , 'enrolled' ] <- 0

cpsc_enrollment_df[ , 'enrolled' ] <- as.numeric( as.character( cpsc_enrollment_df[ , 'enrolled' ] ) )

stopifnot( all( !is.na( cpsc_enrollment_df[ , 'enrolled' ] ) ) )

stopifnot( nrow( cpsc_enrollment_df ) == nrow( unique( cpsc_enrollment_df ) ) )


# overwrite Los Angeles county with the ssa code used in the penetration file
stopifnot( nrow( subset( pen_df , ssa == 5200 ) ) == 1 & nrow( subset( pen_df , ssa == 5210 ) ) == 0 )

cpsc_enrollment_df[ cpsc_enrollment_df[ , 'ssa' ] %in% 5210 , 'ssa' ] <- 5200


# overwrite LaSalle county, Louisiana without a space like the penetration file
stopifnot( 
	all( 
		( pen_df[ pen_df[ , 'ssa' ] == 19290 , 'county' ] %in% 'LaSalle' ) & 
		( cpsc_enrollment_df[ cpsc_enrollment_df[ , 'ssa' ] == 19290 , 'county' ] %in% 'La Salle' )
	)
)

cpsc_enrollment_df[ cpsc_enrollment_df[ , 'ssa' ] %in% 19290 , 'county' ] <- 'LaSalle'



before_enr <- sum( cpsc_enrollment_df[ , 'enrolled' ] )

cpsc_enrollment_df <- subset( cpsc_enrollment_df , !( stateab %in% '' ) )

print( paste( "removed" , before_enr - sum( cpsc_enrollment_df[ , 'enrolled' ] ) , "enrollees out of about" , round( before_enr / 10^6 ) , "million due to missing geography" ) )
# squelching county-plan records with less than 11 enrollees removes much more than this does

# unfortunately, some cpsc records have multiple lines per plan x county
cpsc_enrollment <- cpsc_enrollment_df

# collapse guam fips codes
cpsc_enrollment[ cpsc_enrollment[ , 'stateab' ] %in% "GU" , 'fips' ] <- 66010
cpsc_enrollment[ cpsc_enrollment[ , 'fips' ] %in% 66000:66999 , 'fips' ] <- 66010
cpsc_enrollment[ cpsc_enrollment[ , 'fips' ] %in% 66000:66999 , 'county' ] <- "Guam"
cpsc_enrollment[ cpsc_enrollment[ , 'fips' ] %in% 66000:66999 , 'ssa' ] <- 98000

# collapse northern mariana islands fips codes
cpsc_enrollment[ cpsc_enrollment[ , 'stateab' ] %in% "MP" , 'fips' ] <- 69085
cpsc_enrollment[ cpsc_enrollment[ , 'fips' ] %in% 69000:69999 , 'fips' ] <- 69085
cpsc_enrollment[ cpsc_enrollment[ , 'fips' ] %in% 69000:69999 , 'county' ] <- "Northern Mariana Islands"

# collapse american samoa fips codes
cpsc_enrollment[ cpsc_enrollment[ , 'stateab' ] %in% "AS" , 'fips' ] <- 60000
cpsc_enrollment[ cpsc_enrollment[ , 'fips' ] %in% 60000:60999 , 'fips' ] <- 60000
cpsc_enrollment[ cpsc_enrollment[ , 'fips' ] %in% 60000:60999 , 'ssa' ] <- 64000
cpsc_enrollment[ cpsc_enrollment[ , 'fips' ] %in% 60000:60999 , 'county' ] <- "American Samoa"


cpsc_enrollment_df <- aggregate( enrolled ~ contract_id + plan_id + ssa + fips + stateab + county , data = cpsc_enrollment , sum )

stopifnot( sum( cpsc_enrollment[ , 'enrolled' ] ) == sum( cpsc_enrollment_df[ , 'enrolled' ] ) )

rm( cpsc_enrollment )

# cpsc contract
cpsc_contract_df <- read.csv( cpsc_contract_fn )
names( cpsc_contract_df ) <- c( 'contract_id' , 'plan_id' , 'ot' , 'pt' , 'partd' , 'snp' , 'eghp' , 'oname' , 'mktname' , 'pname' , 'po' , 'datestart' )
cpsc_contract_df <- unique( cpsc_contract_df )

translit_fun <- function( w ) iconv( w , from = 'latin1' , to = 'ASCII//TRANSLIT' )

cpsc_contract_df[ c( 'pname' , 'po' , 'mktname' , 'oname' ) ] <- 
	sapply( 
		cpsc_contract_df[ c( 'pname' , 'po' , 'mktname' , 'oname' ) ] ,
		translit_fun
	)
...
# merge
contract_enrollment_df <- merge( cpsc_contract_df , cpsc_enrollment_df )
stopifnot( nrow( contract_enrollment_df ) == nrow( cpsc_enrollment_df ) )
...
# # # # # # # # # # # # # # # # # # # # # # #
# merge contract/enrollment to availability #
# # # # # # # # # # # # # # # # # # # # # # #


# merge both ma & pdp availability tables with cpsc enrollment
pre_merge_ma_df <- ma_availability_df
pre_merge_pdp_df <- pdp_availability_df

pre_merge_ma_df[ , 'ma_availability_match' ] <- TRUE
pre_merge_pdp_df[ , 'pdp_availability_match' ] <- TRUE


for( this_variable in c( "pt", "partd", "snp", "mktname", "pname", "po", "state_name", "stateab" , "county" , "ssa" ) ){

	pre_merge_ma_df[ , paste0( 'pre_' , this_variable ) ] <- pre_merge_ma_df[ , this_variable ]
	pre_merge_ma_df[ , this_variable ] <- NULL
	
	pre_merge_pdp_df[ , paste0( 'pre_' , this_variable ) ] <- pre_merge_pdp_df[ , this_variable ]
	pre_merge_pdp_df[ , this_variable ] <- NULL
	
}


# confirm contract+plan+fips gives the same result as contract+plan+state+county+ssa+fips
post_merge_ma_df <-
	merge(
		contract_enrollment_df[ substr( contract_enrollment_df[ , 'contract_id' ] , 1 , 1 ) %in% c( 'H' , 'R' ) , ] , 
		pre_merge_ma_df ,
		all.x = TRUE ,
		by = c( 'contract_id' , 'plan_id' , 'fips' )
	)
...
test_merge_ma_df <-
	merge(
		...
		pre_merge_ma_df ,
		all.x = TRUE ,
		by.x = c( 'contract_id' , 'plan_id' , 'stateab' , 'county' , 'ssa' , 'fips' ) ,
		by.y = c( 'contract_id' , 'plan_id' , 'pre_stateab' , 'pre_county' , 'pre_ssa' , 'fips' )
	)


stopifnot( nrow( post_merge_ma_df ) == nrow( test_merge_ma_df ) )

stopifnot( sum( is.na( post_merge_ma_df[ , 'pre_state_name' ] ) ) == sum( is.na( test_merge_ma_df[ , 'pre_state_name' ] ) ) )
...
post_merge_pdp_df <-
	merge(
		contract_enrollment_df[ contract_enrollment_df[ , 'partd' ] == 'Yes' , ] , 
		pre_merge_pdp_df ,
		all.x = TRUE ,
		by = c( 'contract_id' , 'plan_id' , 'fips' )
	)
...
stopifnot( nrow( post_merge_pdp_df ) == nrow( test_merge_pdp_df ) )

stopifnot( sum( is.na( post_merge_pdp_df[ , 'pre_state_name' ] ) ) == sum( is.na( test_merge_pdp_df[ , 'pre_state_name' ] ) ) )

rm( test_merge_pdp_df )

# confirm zero enrollment changes
stopifnot( sum( contract_enrollment_df[ substr( contract_enrollment_df[ , 'contract_id' ] , 1 , 1 ) %in% c( 'H' , 'R' ) , 'enrolled' ] ) == sum( post_merge_ma_df[ , 'enrolled' ] ) )

stopifnot( sum( contract_enrollment_df[ contract_enrollment_df[ , 'partd' ] == 'Yes' , 'enrolled' ] ) == sum( post_merge_pdp_df[ , 'enrolled' ] ) )

post_merge_ma_df <- subset( post_merge_ma_df , enrolled > 0 )

post_merge_pdp_df <- subset( post_merge_pdp_df , enrolled > 0 )

post_merge_ma_nrow <- nrow( post_merge_ma_df )

post_merge_pdp_nrow <- nrow( post_merge_pdp_df )

post_merge_ma_df[ is.na( post_merge_ma_df[ , 'ma_availability_match' ] ) , 'ma_availability_match' ] <- FALSE

post_merge_pdp_df[ is.na( post_merge_pdp_df[ , 'pdp_availability_match' ] ) , 'pdp_availability_match' ] <- FALSE

post_merge_ma_df[ c( 'pre_stateab' , 'pre_county' , 'pre_ssa' ) ] <- NULL

post_merge_pdp_df[ c( 'pre_stateab' , 'pre_county' , 'pre_ssa' ) ] <- NULL

pre_merge_ma_df[ c( 'pre_stateab' , 'pre_county' , 'pre_ssa' ) ] <- NULL

pre_merge_pdp_df[ c( 'pre_stateab' , 'pre_county' , 'pre_ssa' ) ] <- NULL



# fill in plan-wide and county-wide variables where missing (this still misses eghps)

cpl <- nrow( unique( pre_merge_ma_df[ c( 'contract_id' , 'plan_id' ) ] ) )

countyl <- nrow( unique( pre_merge_ma_df[ 'fips' ] ) )


for( this_variable in names( pre_merge_ma_df ) ){

	contract_plan_variable <- unique( pre_merge_ma_df[ c( 'contract_id' , 'plan_id' , this_variable ) ] )
	
	fips_variable <- unique( pre_merge_ma_df[ c( 'fips' , this_variable ) ] )

	if( cpl == nrow( contract_plan_variable ) ){
		
		names( contract_plan_variable )[ 3 ] <- paste0( 'merged_' , this_variable )
		
		post_merge_ma_df <- merge( post_merge_ma_df , contract_plan_variable , all.x = TRUE )
		
		post_merge_ma_df[ is.na( post_merge_ma_df[ , this_variable ] ) , this_variable ] <-
			post_merge_ma_df[ is.na( post_merge_ma_df[ , this_variable ] ) , paste0( 'merged_' , this_variable ) ]
			
		post_merge_ma_df[ , paste0( 'merged_' , this_variable ) ] <- NULL
		
	}
	
	if( countyl == nrow( fips_variable ) ){
	
		names( fips_variable )[ 2 ] <- paste0( 'merged_' , this_variable )
		
		post_merge_ma_df <- merge( post_merge_ma_df , fips_variable , all.x = TRUE )
...
cpl <- nrow( unique( pre_merge_pdp_df[ c( 'contract_id' , 'plan_id' ) ] ) )

countyl <- nrow( unique( pre_merge_pdp_df[ 'fips' ] ) )


for( this_variable in names( pre_merge_pdp_df ) ){

	contract_plan_variable <- unique( pre_merge_pdp_df[ c( 'contract_id' , 'plan_id' , this_variable ) ] )
	
	fips_variable <- unique( pre_merge_pdp_df[ c( 'fips' , this_variable ) ] )

	if( cpl == nrow( contract_plan_variable ) ){
		
		names( contract_plan_variable )[ 3 ] <- paste0( 'merged_' , this_variable )
		
		post_merge_pdp_df <- merge( post_merge_pdp_df , contract_plan_variable , all.x = TRUE )
		
		post_merge_pdp_df[ is.na( post_merge_pdp_df[ , this_variable ] ) , this_variable ] <-
			post_merge_pdp_df[ is.na( post_merge_pdp_df[ , this_variable ] ) , paste0( 'merged_' , this_variable ) ]
			
		post_merge_pdp_df[ , paste0( 'merged_' , this_variable ) ] <- NULL
	...
	if( countyl == nrow( fips_variable ) ){
		...
		post_merge_pdp_df <- merge( post_merge_pdp_df , fips_variable , all.x = TRUE )
...
stopifnot( nrow( post_merge_ma_df ) == post_merge_ma_nrow )

stopifnot( nrow( post_merge_pdp_df ) == post_merge_pdp_nrow )

# some plans without a match (for example eghps) might still be missing segment_id
post_merge_ma_df[ is.na( post_merge_ma_df[ , 'segment_id' ] ) , 'segment_id' ] <- 0

post_merge_pdp_df[ is.na( post_merge_pdp_df[ , 'segment_id' ] ) , 'segment_id' ] <- 0

post_merge_ma_df[ is.na( post_merge_ma_df[ , 'contract_year' ] ) , 'contract_year' ] <- year

post_merge_pdp_df[ is.na( post_merge_pdp_df[ , 'contract_year' ] ) , 'contract_year' ] <- year

post_merge_ma_df[ , 'mapd_standalone' ] <- 
	ifelse( post_merge_ma_df[ , 'partd' ] == 'No' , 'ma only' ,
	ifelse( post_merge_ma_df[ , 'pt' ] %in% c( 'Medicare Prescription Drug Plan' , 'Employer/Union Only Direct Contract PDP' ) , 'standalone pdp' , 'ma-pd' ) )

post_merge_pdp_df[ , 'mapd_standalone' ] <- 
	ifelse( post_merge_pdp_df[ , 'pt' ] %in% c( 'Medicare Prescription Drug Plan' , 'Employer/Union Only Direct Contract PDP' ) , 'standalone pdp' , 'ma-pd' )

# fill in zeroes for standalone pdp part c premiums
post_merge_pdp_df[ post_merge_pdp_df[ , 'mapd_standalone' ] %in% 'standalone pdp' , 'partc_premium' ] <- 0

post_merge_pdp_df[ post_merge_pdp_df[ , 'mapd_standalone' ] %in% 'standalone pdp' , 'premium' ] <-
	post_merge_pdp_df[ post_merge_pdp_df[ , 'mapd_standalone' ] %in% 'standalone pdp' , 'partd_premium' ]




# merge the contract directory again because eghps missed previously, these both should be 100% matches

before_nrow <- nrow( post_merge_pdp_df )

post_merge_pdp_df[ , setdiff( names( contract_directory_df ) , 'contract_id' ) ] <- NULL

post_merge_pdp_df <- merge( post_merge_pdp_df , contract_directory_df )

stopifnot( nrow( post_merge_pdp_df ) == before_nrow )

before_nrow <- nrow( post_merge_ma_df )

post_merge_ma_df[ , setdiff( names( contract_directory_df ) , 'contract_id' ) ] <- NULL

post_merge_ma_df <- merge( post_merge_ma_df , contract_directory_df )

stopifnot( nrow( post_merge_ma_df ) == before_nrow )


# pre_state_name was not included in the merge test so re-name it here instead
names( post_merge_ma_df )[ names( post_merge_ma_df ) == 'pre_state_name' ] <- 'state_name'

names( post_merge_pdp_df )[ names( post_merge_pdp_df ) == 'pre_state_name' ] <- 'state_name'


# re-running the cost sharing & drug tier merge isn't necessary here because eghps aren't included in those files:
stopifnot( all( this_rx_cost_sharing[ , 'plan_id' ] < 800 ) )


if( Sys.Date() > '2026-07-01' ) {
	
	stop( "review whether these differences matter" )
	
	for( this_variable in c( "pt", "partd", "snp", "mktname", "pname", "po") ){
		print( paste( 'post_merge_ma_df' , this_variable ) )
		print( unique( post_merge_ma_df[ !is.na( post_merge_ma_df[ , paste0( 'pre_' , this_variable ) ] ) & post_merge_ma_df[ , paste0( 'pre_' , this_variable ) ] != post_merge_ma_df[ , this_variable ] , c(this_variable,paste0( 'pre_' , this_variable ))]))

		print( paste( 'post_merge_pdp_df' , this_variable ) )
		print( unique( post_merge_pdp_df[ !is.na( post_merge_pdp_df[ , paste0( 'pre_' , this_variable ) ] ) & post_merge_pdp_df[ , paste0( 'pre_' , this_variable ) ] != post_merge_pdp_df[ , this_variable ] , c(this_variable,paste0( 'pre_' , this_variable ))]))
	}
	
} else {
	
	# if they don't matter, delete the information from the availability file with these two lines
	
	post_merge_ma_df[ , paste0( 'pre_' , c( "pt", "partd", "snp", "mktname", "pname", "po") ) ] <- NULL
	
	post_merge_pdp_df[ , paste0( 'pre_' , c( "pt", "partd", "snp", "mktname", "pname", "po") ) ] <- NULL
...
# # # # # # # # # # # # # #
# add pdp_region variable #
# # # # # # # # # # # # # #

before_nrow <- nrow( post_merge_pdp_df )
post_merge_pdp_df[ , 'pdp_region' ] <- post_merge_ma_df[ , 'pdp_region' ] <- NULL
post_merge_pdp_df <- merge( post_merge_pdp_df , pdp_regions )
stopifnot( nrow( post_merge_pdp_df ) == before_nrow )
...
post_merge_pdp_df[ , c( "tiered_pharmacy" , "mrx_form_model_type" , "mrx_form_model_desc" ) ] <- 
	post_merge_ma_df[ , c( "tiered_pharmacy" , "mrx_form_model_type" , "mrx_form_model_desc" ) ] <- NULL

post_merge_pdp_df <- merge( post_merge_pdp_df , pbp_mrx_df , all.x = TRUE )

stopifnot( nrow( post_merge_pdp_df ) == before_nrow )

# # # # # # # # # # # # # # # # # # # # #
# merge benefits table to ma enrollment #
# # # # # # # # # # # # # # # # # # # # #

before_nrow <- nrow( post_merge_ma_df )

variables_to_remerge <- setdiff( names( benefits_df ) , c( 'contract_id' , 'plan_id' , 'segment_id' ) )

benefits_df[ , 'in_benefits_df' ] <- TRUE

post_merge_ma_df[ variables_to_remerge ] <- NULL

post_merge_ma_df <- merge( post_merge_ma_df , benefits_df , all.x = TRUE )

post_merge_ma_df[ is.na( post_merge_ma_df[ , 'in_benefits_df' ] ) , 'in_benefits_df' ] <- FALSE
...
# confirm plan type exclusions
pt_options <- c("Medicare Prescription Drug Plan","Employer/Union Only Direct Contract PDP" ,"Medicare-Medicaid Plan HMO/HMOPOS", "HMO/HMOPOS", "Local PPO", "National PACE", "MSA", "1876 Cost", "PFFS", "Regional PPO" , "Employer/Union Only Direct Contract PFFS" , "PSO (State License)" , "MA Health Senior Care Options", "MN Disability Health Options", "MN Senior Health Options", "SHMO", "PSO (Federal Waiver of State License)", "WI Partnership Program", "ESRD II", "ESRD I", "Continuing Care Retirement Community", "HCPP - 1833 Cost" , "Pilot" , "RFB PFFS")

stopifnot( all( post_merge_ma_df[ , 'pt' ] %in% pt_options ) )

stopifnot( all( post_merge_pdp_df[ , 'pt' ] %in% pt_options ) )

# exclude missing states AND medicare-medicaid plans, 1876 cost plans, and national pace plans
ma_enrollment_df <- 
	subset( 
		post_merge_ma_df , 
		!( pt %in% c( "National PACE" , "1876 Cost" , 'Medicare-Medicaid Plan HMO/HMOPOS' , "HCPP - 1833 Cost" , "MA Health Senior Care Options" , "WI Partnership Program" , "Pilot" ) ) 
		& stateab %in% c( state.abb , "DC" , "PR" , "AS" , "GU" , "MP" , "VI" )
	)
...
ma_enrollment_df[ , 'state_name' ] <- 
	c( state.name , "Washington D.C." , "Puerto Rico" , "American Samoa" , "Guam" , "Northern Mariana Islands" , "Virgin Islands" )[ 
		match( ma_enrollment_df[ , 'stateab' ] , c( state.abb , "DC" , "PR" , "AS" , "GU" , "MP" , "VI" ) ) 
	]


pdp_enrollment_df[ , 'state_name' ] <- 
	c( state.name , "Washington D.C." , "Puerto Rico" , "American Samoa" , "Guam" , "Northern Mariana Islands" , "Virgin Islands" )[ 
		match( pdp_enrollment_df[ , 'stateab' ] , c( state.abb , "DC" , "PR" , "AS" , "GU" , "MP" , "VI" ) ) 
	]

if( any( c( is.na( ma_enrollment_df[ , 'state_name' ] ) , is.na( pdp_enrollment_df[ , 'state_name' ] ) ) ) ) stop( 'should match 100% given subset above' )
	
	

# 18k enrollees in counties where plans do not operate
# 6m enrollees in ma eghps which aren't in the landscape file
# 30m matches
with( ma_enrollment_df , tapply( enrolled , list( eghp , ma_availability_match ) , sum , na.rm = TRUE ) )


# 53k enrollees in counties where plans do not operate
# 9m enrollees in ma eghps which aren't in the landscape file
# 47m matches
with( pdp_enrollment_df , tapply( enrolled , list( eghp , pdp_availability_match ) , sum , na.rm = TRUE ) )


# # # # # # # # # # # # # # # # # #
# third and fourth of four saves  #
# # # # # # # # # # # # # # # # # #


saveRDS( ma_enrollment_df , file = paste0( "R:/Medicare Tracker/output/" , year , "03 medicare advantage county plan enrollment.rds" ) )

saveRDS( pdp_enrollment_df , file = paste0( "R:/Medicare Tracker/output/" , year , "03 pdp county plan enrollment.rds" ) )
